...
class TestSearchPost:
    ...
    def test_valid_query_creates_job(self, client):
        with patch("ai_researcher.app.research_graph.run", return_value={"report": "/reports/r.pdf", "research_results": ""}):
...
class TestSearchStatus:
    ...
    def test_completed_job_includes_result(self, client):
        ...
        with jobs_lock:
            jobs[job_id] = {
                ...
                "result": {
                    "query": "NLP trends",
                    "research_results": "some findings",
        ...
        assert data["research_results"] == "some findings"
    ...
    def test_failed_job_includes_error(self, client):
        ...
        assert data["error"] == "SERPAPI_KEY not set"
